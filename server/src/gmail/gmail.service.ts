...
export class GmailService {
  ...
  private readonly userStorePath = path.join(process.cwd(), 'data', 'users.json');
  ...
  async handlePushNotification(reqBody: any) {
    ...
    try {
      ...
      const msg = JSON.parse(Buffer.from(reqBody.message.data, 'base64').toString());
  ...
  async createDraftReply(messageId: string, userEmail: string, gmail: any) {
    ...
    const message = await gmail.users.messages.get({
      ...
      format: 'metadata',
      metadataHeaders: [
        'Subject', 'Message-ID', 'References', 'In-Reply-To',
        'From', 'To', 'Cc'
      ],
    });
    ...
    this.logger.log(`Draft created in thread: ${threadId}, draftId: ${draft.data.id}`);
  }
  ...
  private async extractEmailBody(messageId: string, gmail: any): Promise<string> {
  ...
  private getGmailClientFromTokens(tokens: any) {
    const oauth2 = new google.auth.OAuth2(this.CLIENT_ID, this.CLIENT_SECRET, 'http://localhost:8080/gmail/auth/callback');
